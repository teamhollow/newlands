...
public class NLBoatItem extends Item {
    private static final Predicate<Entity> RIDERS = EntityPredicates.EXCEPT_SPECTATOR.and(Entity::collides);
    private final Supplier<EntityType<NLBoatEntity>> boatSupplier;

    public NLBoatItem(Supplier<EntityType<NLBoatEntity>> boatSupplier, Item.Settings settings) {
        super(settings);
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        NLBoatEntity boat = createBoat(world, hit.getPos().x, hit.getPos().y, hit.getPos().z);
    ...
    private NLBoatEntity createBoat(World world, double x, double y, double z) {
        NLBoatEntity entity = boatSupplier.get().create(world);
        if (entity != null) {
            entity.setPos(x, y, z);
