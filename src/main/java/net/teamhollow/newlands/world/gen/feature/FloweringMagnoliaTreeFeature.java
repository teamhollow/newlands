...
public class FloweringMagnoliaTreeFeature extends Feature<TreeFeatureConfig> {
    public FloweringMagnoliaTreeFeature(Codec<TreeFeatureConfig> codec) {
        super(codec);
    }
    ...
    private static boolean isVine(TestableWorld world, BlockPos pos) {
        return world.testBlockState(pos, (state) -> {
            return state.isOf(NLBlocks.MAGNOLIA_VINE);
        });
    }
        ...
        if (blockPos2.getY() >= 1 && blockPos2.getY() + i + 1 <= 256) {
            if (!isDirtOrGrass(world, blockPos2.down())) {
                ...
                if (r >= i || optionalInt.isPresent() && r >= optionalInt.getAsInt()) {
                    List<FoliagePlacer.TreeNode> list = config.trunkPlacer.generate(world, random, r, blockPos2,
                            logPositions, box, config);
                    list.forEach((treeNode) -> {
                        config.foliagePlacer.generate(world, random, config, this.method_29963(world, i, blockPos2, config), treeNode, j, l, leavesPositions, box);
                    });
                    return true;
                } else {
                    return false;
            ...
            for (int var13 = 0; var13 < var12; ++var13) {
                ...
                if (!logs.contains(mutable)) {
                    ...
                    if (blockState.contains(Properties.DISTANCE_1_7)) {
                        ((Set<BlockPos>) list.get(0)).add(mutable.toImmutable());
                        setBlockStateWithoutUpdatingNeighbors(world, mutable,
                                (BlockState) blockState.with(Properties.DISTANCE_1_7, 1));
                        if (box.contains(mutable)) {
                            voxelSet.set(mutable.getX() - box.minX, mutable.getY() - box.minY,
                                    mutable.getZ() - box.minZ, true, true);
                        }
                    }
                }
            }
