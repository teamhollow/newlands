...
public class NLConfiguredFeatures {
    ...
    public static final ConfiguredFeature<TreeFeatureConfig, ?> TREE_MAGNOLIA_FLOWERING = register("tree_magnolia_flowering", NLFeatures.TREE_FLOWERING_MAGNOLIA.configure(new TreeFeatureConfig.Builder(new SimpleBlockStateProvider(States.MAGNOLIA_LOG), new WeightedBlockStateProvider().addState(States.MAGNOLIA_LEAVES, 3).addState(States.FLOWERING_MAGNOLIA_LEAVES, 1), new LargeOakFoliagePlacer(UniformIntDistribution.of(2), UniformIntDistribution.of(4), 4), new LargeOakTrunkPlacer(9, 11, 0), new TwoLayersFeatureSize(0, 0, 0, OptionalInt.of(4))).heightmap(Heightmap.Type.MOTION_BLOCKING).decorators(ImmutableList.of(LeaveMagnoliaVineTreeDecorator.INSTANCE)).ignoreVines().build()));
