...
public class SandLayerFeature extends Feature<ProbabilityConfig> {
	...
	public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, ProbabilityConfig config) {
		for (int x=0;x<16;x++) {
			for (int z = 0; z < 16; z++) {
				BlockPos pos1 = new BlockPos(pos.getX() + x, worldIn.getHeight(Heightmap.Type.OCEAN_FLOOR, pos.add(x, 0, z)).getY(), pos.getZ() + z);
	...
	public int getLayerHeight(IWorld world,BlockPos pos) {
		int y=world.getHeight(Heightmap.Type.OCEAN_FLOOR,pos).getY();
		float height=0;
		for (int x2=-3;x2<=3;x2++) {
			for (int z2=-3;z2<=3;z2++) {
				int y2=world.getHeight(Heightmap.Type.OCEAN_FLOOR,pos.add(x2,0,z2)).getY();
				if (y2>y) {
					height+=2;
				}
				height/=1.2f;
