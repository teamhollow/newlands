...
public class TropicalFishermanModel extends EntityModel<Entity> {
	...
	private final ModelRenderer bodyNoArms;
	...
	public TropicalFishermanModel() {
		...
		bodyNoArms = new ModelRenderer(this);
		bodyNoArms.setRotationPoint(0.0F, 0.0F, 0.0F);
		bodyNoArms.setTextureOffset(16, 20).addBox(-4.0F, 0.0F, -3.0F, 8.0F, 12.0F, 6.0F, 0.0F, false);
		bodyNoArms.setTextureOffset(0, 40).addBox(-4.0F, 0.0F, -3.0F, 8.0F, 18.0F, 6.0F, 0.25F, false);
		...
		bodyNoArms.addChild(left_leg);
		bodyNoArms.addChild(right_leg);
		body.addChild(left_arm);
		body.addChild(right_arm);
		body.addChild(bodyNoArms);
...
//		body.addChild(right_leg);
	...
	public void setRotationAngles(Entity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float partialTicks) {
		//GET LIMB SWING
		float limbSwingVal = MathHelper.cos((limbSwing + (partialTicks / 2f)) * 0.6662F) * 2.0F * limbSwingAmount;
		...
		if (entity.getRidingEntity() != null) {
			...
			if (entity.getRidingEntity() instanceof BoatEntity && entity.getRidingEntity().getPassengers().size() == 1) {
				...
				float rowTime = boat.getRowingTime(0, limbSwing) + partialTicks * 0.5f;
				setRotationAngle(right_arm, (float) Math.toRadians(Math.cos(rowTime) * 12 - 75), (float) Math.sin(-rowTime) / 12f, 0);
				rowTime = boat.getRowingTime(1, limbSwing) + partialTicks * 0.5f;
				...
				if (((VillagerEntity) entity).getBrain().getSchedule().getScheduledActivity((int) entity.world.getDayTime()).equals(Activity.WORK)&& FishermanEntity.checkBoat((FishermanEntity)entity)) {
					...
					if (!(entity.ticksExisted % 200 <= 40)) {
						//FISHING
						setRotationAngle(right_arm, (float) Math.toRadians(-80 + xOff), (float) Math.toRadians(3 + zOff), 0);
					} else {
						int returnOffset = 36;
			...
			setRotationAngle(right_arm, limbSwingVal * -0.5F + (float) Math.toRadians(xOff), -(float) Math.toRadians(zOff), 0);
			setRotationAngle(left_arm, limbSwingVal * 0.5f - (float) Math.toRadians(xOff), (float) Math.toRadians(zOff), 0);
			setRotationAngle(right_leg, limbSwingVal * 0.5F, 0, 0);
			setRotationAngle(left_leg, limbSwingVal * -0.5F, 0, 0);
...
//		arms.render(matrixStack, buffer, packedLight, packedOverlay);
	...
	public void render(MatrixStack matrixStack, IVertexBuilder buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha, boolean doingAnything) {
		head.render(matrixStack, buffer, packedLight, packedOverlay, red, green, blue, alpha);
		nose.render(matrixStack, buffer, packedLight, packedOverlay, red, green, blue, alpha);
		if (!doingAnything) {
			bodyNoArms.render(matrixStack, buffer, packedLight, packedOverlay, red, green, blue, alpha);
